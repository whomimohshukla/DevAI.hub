...
const textCompletion = async (req, res) => {
	...
	let pm = await providermodel_model_1.ProviderModel.findById(
		route.defaultProviderModelId
	);
	...
	if (provider.name === "openai") {
		const out = await (0, openai_1.openaiTextCompletion)(
			prompt,
			pm.modelName,
			params
		);
...
const imageGenerate = async (req, res) => {
	...
	let latencyMs = 0;
	if (provider.name === "openai") {
		const out = await (0, openai_1.openaiImageGenerate)(
			prompt,
			pm.modelName,
			params
		);
...
const speech = async (req, res) => {
	...
	if (action === "stt") {
		...
		if (audioBase64 && typeof audioBase64 === "string") {
			...
			return res
				.status(400)
				.json({ error: "Provide audioBase64 or audioUrl" });
		}
		const out = await (0, openai_1.openaiSpeechToText)(
			audioBuf,
			pm.modelName,
			params
		);
		...
		if (!text || typeof text !== "string") {
			return res
				.status(400)
				.json({ error: "text (string) is required for tts" });
		}
		const out = await (0, openai_1.openaiTextToSpeech)(
			text,
			pm.modelName,
			voice,
			format
