...
const listProviderModels = async (_req, res) => {
    const items = await providermodel_model_1.ProviderModel.find()
        .sort({ service: 1, modelName: 1 })
...
const createProviderModel = async (req, res) => {
    ...
    if (!providerId || !modelName || !service) {
        return res
            .status(400)
            .json({ error: "providerId, modelName, service are required" });
    }
    const created = await providermodel_model_1.ProviderModel.create({
        providerId,
        modelName,
        service,
        defaultParams,
...
const updateProviderModel = async (req, res) => {
    const { id } = req.params;
    const updated = await providermodel_model_1.ProviderModel.findByIdAndUpdate(id, req.body, {
        new: true,
    });
    if (!updated)
