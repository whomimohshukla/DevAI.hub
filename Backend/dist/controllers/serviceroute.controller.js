...
const listServiceRoutes = async (_req, res) => {
    const items = await serviceroute_model_1.ServiceRoute.find()
        .sort({ service: 1, routeName: 1 })
        .populate("defaultProviderModelId", "modelName service");
...
const createServiceRoute = async (req, res) => {
    ...
    if (!service || !routeName || !defaultProviderModelId) {
        return res.status(400).json({
            error: "service, routeName, defaultProviderModelId are required",
        });
    }
    const created = await serviceroute_model_1.ServiceRoute.create({
        service,
        routeName,
...
const updateServiceRoute = async (req, res) => {
    const { id } = req.params;
    const updated = await serviceroute_model_1.ServiceRoute.findByIdAndUpdate(id, req.body, {
        new: true,
    });
    if (!updated)
