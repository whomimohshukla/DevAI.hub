...
export const listProviderModels = async (_req: Request, res: Response) => {
  const items = await ProviderModel.find()
...
export const createProviderModel = async (req: Request, res: Response) => {
  const { providerId, modelName, service, defaultParams, pricing, status } =
    req.body || {};
...
export const updateProviderModel = async (req: Request, res: Response) => {
  const { id } = req.params;
  const updated = await ProviderModel.findByIdAndUpdate(id, req.body, {
    new: true,
  });
  if (!updated)
